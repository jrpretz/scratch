...
def untrained_weights():
    w_1 = tf.Variable(tf.random_normal((2,24), stddev=0.1))
...
x1 = np.random.random(size=400) - 1.
x2 = np.random.random(size=400) -1. 
X_train[0:400,0] = x1
X_train[0:400,1] = x2
y_train[0:400,0] = np.zeros(shape=400)

x1 = np.random.random(size=400) - 1.
x2 = np.random.random(size=400)
...
plt.colorbar(c)
plt.savefig("train-xor-again.png")
